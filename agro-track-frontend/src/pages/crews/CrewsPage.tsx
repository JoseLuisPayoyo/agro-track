...
export default function CrewsPage() {
  ...
  const onSubmit = async () => {
    try {
      if (model.id) {
        await crewsService.update(model.id, model)
        toast.success('Cuadrilla actualizada')
      } else {
        await crewsService.create(model)
    ...
    <div className="space-y-4">
      <header className="flex items-center justify-between">
        ...
        <button className="px-3 py-2 bg-brand-600 text-white rounded-lg"
          onClick={()=>{setModel(empty); setOpen(true)}}>Añadir cuadrilla</button>
      </header>
      ...
      <ConfirmDialog open={confirm.open} onCancel={()=>setConfirm({open:false})} onConfirm={onDelete}
        title="Eliminar cuadrilla" message="Esta acción no se puede deshacer." />
    </div>
  )
}
